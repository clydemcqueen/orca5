...
class StereoSlamBridge(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        self.conn = pymavlink.mavutil.mavlink_connection(
            mav_device,
            source_system=StereoSlamBridge.SOURCE_SYSTEM,
            source_component=StereoSlamBridge.SOURCE_COMPONENT)
        ...
        # Subscriptions
        self.map_sub = self.create_subscription(sensor_msgs.msg.PointCloud2, '/orbslam3/pointcloud', self.map_callback, 10)
        self.pose_sub = self.create_subscription(geometry_msgs.msg.PoseStamped, '/orbslam3/pose', self.pose_callback, 10)
        self.slam_sub = self.create_subscription(orbslam3_msgs.msg.SlamStatus, '/orbslam3/slam_status', self.slam_callback, 10)
        ...
        self.receive_pose = False
        self.receive_pcl = False
    ...
    def publish_static_transforms(self):
        ...
        tf_msg.child_frame_id = 'left_camera_sensor'
    ...
    def slam_callback(self, msg: orbslam3_msgs.msg.SlamStatus):

        #----------
        # Wait for everything to warm up
        #----------

        if self.receive_pose or self.receive_pcl:
            # self.get_logger().info('receive pose message')
            ...
            if msg.tracking_state != orbslam3_msgs.msg.SlamStatus.TRACKING_OK:
                if self.tracking:
                    self.tracking = False
        ...
        if not self.receive_pose or not self.receive_pcl:
            self.get_logger().warn('not receive pose yet')
            return

    def map_callback(self, msg: sensor_msgs.msg.PointCloud2):
        """Scale the map and republish it."""
        self.pcl = msg
        self.receive_pcl = True
